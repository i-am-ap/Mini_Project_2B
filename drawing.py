...
class Paint(object):
    ...
    def get_fingertip_coordinates():
        try:
            with open("fingertip_coordinates.txt", "r") as f:
    ...
    def update(self):
        ...
        # x, y = get_fingertip_coordinates()
        # if x is not None and y is not None:
        #     if self.old_x and self.old_y:
        #         self.c.create_line(self.old_x, self.old_y, x, y, width=self.line_width, fill=self.color, capstyle=ROUND, smooth=TRUE, splinesteps=36)
        #     self.old_x, self.old_y = x, y
        # self.root.after(10, self.update)  # schedule next update in 10 ms
